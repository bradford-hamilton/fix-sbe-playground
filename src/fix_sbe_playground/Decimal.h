...
#ifndef _FIX_SBE_PLAYGROUND_DECIMAL_CXX_H_
#define _FIX_SBE_PLAYGROUND_DECIMAL_CXX_H_
...
public:
    ...
    Decimal(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        Decimal(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    Decimal(
        char *buffer,
        const std::uint64_t bufferLength) :
        Decimal(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    Decimal &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = Decimal(buffer, offset, bufferLength, actingVersion);
    }
    ...
    static SBE_CONSTEXPR std::uint64_t mantissaNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_UINT64;
    }

    static SBE_CONSTEXPR std::uint64_t mantissaMinValue() SBE_NOEXCEPT
    {
        return UINT64_C(0x0);
    }

    static SBE_CONSTEXPR std::uint64_t mantissaMaxValue() SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD std::uint64_t mantissa() const SBE_NOEXCEPT
    {
        std::uint64_t val;
    ...
    Decimal &mantissa(const std::uint64_t value) SBE_NOEXCEPT
    {
    ...
    Decimal &exponent(const std::int8_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, Decimal &writer)
{
    builder << '{';
    builder << R"("mantissa": )";
